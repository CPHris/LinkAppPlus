...
const UserPage: NextPage<{ user: User }> = ({ user }) => {
  ...
  const addNewPage = async () => {
    ...
    const response = await http.addNewPage({
  ...
  const deleteLinkPage = async (pageid: string) => {
    const response = await http.deletePage({
    ...
    <>
      <Layout username={stateUser.username} email={stateUser.email}>
        <main className="max-w-4xl mx-auto mb-auto w-full py-10">
          <h1 className="font-bold text-xl text-center mb-5">My Link Pages</h1>
          <LinkPagesList
            linkpages={stateUser.linkPages}
            userRoute={userRoute}
            deleteLinkPage={deleteLinkPage}
          />
          <button
            className="mx-auto block w-fit py-1 px-3 text-gray-700 font-semibold rounded-lg hover:bg-gray-200"
            type="button"
            onClick={addNewPage}
          >
            Add New Page
