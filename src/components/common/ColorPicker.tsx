...
export default function ColorPicker (props: IColorPickerProps) {
  ...
  return (
    <div className={`relative ${className}`}>
      <button type='button' className='flex items-center border rounded-md w-fit p-2'
        onClick={() => { setEditMode(prevState => !prevState); }}
      >
        <div className='w-5 h-5 rounded border' style={{ 'backgroundColor': color }}></div>
        {props.label && <p className='block ml-2'>{props.label}</p>}
      </button>
      {editMode &&
        <SketchPicker className='absolute top-10'
          color={color}
